...
export function WalletControl() {
  ...
  if (wallet.mode !== "injected") {
    return (
      <div className="flex flex-col items-start gap-1 strip:items-end">
        <button
          type="button"
          className="hf-btn"
        ...
        {wallet.error ? (
          <p className="hf-note max-w-[34ch] text-right strip:text-right">{wallet.error}</p>
        ) : !wallet.hasInjected ? (
          <p className="hf-note max-w-[34ch]">
  ...
  return (
    <div className="flex flex-col items-start gap-1 strip:items-end">
      <div className="flex flex-wrap items-center gap-2">
        <span className="hf-record" title={wallet.address}>
          {shortenHex(wallet.address ?? "")}
        </span>
        <span className="hf-label">{wallet.networkName}</span>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        {wallet.network.kind !== "expected" ? (
          <button type="button" className="hf-btn-quiet" onClick={() => void wallet.switchNetwork()}>
            Switch network
          </button>
        ) : null}
        <button type="button" className="hf-btn-quiet" onClick={wallet.disconnect}>
          Disconnect wallet
        </button>
      </div>
      {wallet.error ? <p className="hf-note max-w-[38ch]">{wallet.error}</p> : null}
    </div>
  );
}
